/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Two environment variables (Netlify > Site configuration > Environment):
   ...
   Scheduled functions have no public URL in production. Trigger it by hand
   with the "Run now" button on the Functions page, and read the outcome in
   the Function log underneath. Set an optional DRY_RUN=1 variable to make a
   run report what it WOULD do without writing anything.
   ═══════════════════════════════════════════════════════════════════════════ */
...
export function buildUpdates(apiMatches, existing){
...
export default async (req) => {
  ...
  /* A scheduled function has no public URL in production, so the dry switch
     is an environment variable. Set DRY_RUN=1 to look without touching. */
  let dry = process.env.DRY_RUN === "1";
  try { if (new URL(req.url).searchParams.get("dry") === "1") dry = true; } catch {}

  const say = (...a) => console.log("[fetch-results]", ...a);

  if (!token || !dbUrl){
    say("MISSING CONFIG — set FOOTBALL_DATA_TOKEN and FIREBASE_DB_URL, then redeploy");
  ...
  try {
    ...
    if (!api.ok){
      say(`FOOTBALL-DATA ERROR ${api.status}` +
          (api.status === 403 ? " — token rejected, check FOOTBALL_DATA_TOKEN" :
           api.status === 429 ? " — rate limited, it will retry on the next run" : ""));
      return Response.json({ ok:false, error:`football-data ${api.status}` }, { status:502 });
    }
    ...
    if (!cur.ok){
      say(`FIREBASE READ ERROR ${cur.status} — check FIREBASE_DB_URL`);
      return Response.json({ ok:false, error:`firebase ${cur.status}` }, { status:502 });
    }
    ...
    say(`to add: ${ids.length}${ids.length ? " -> " + ids.join(", ") : ""}`);
    ...
    if (dry){
      ...
      if (!put.ok){
        ...
        return Response.json({ ok:false, error:`firebase ${put.status}` }, { status:502 });
      }
    ...
    return Response.json({
      ok:true, dryRun:dry,
      finishedInApi:(data.matches || []).length,
      added: dry ? 0 : ids.length, wouldAdd: ids,
      alreadyHad: skipped.length, couldNotMap: unmapped
    });
  } catch (e){
    say("UNEXPECTED ERROR:", String(e && e.message || e));
    return Response.json({ ok:false, error:String(e && e.message || e) }, { status:500 });
  }
};

/* every three hours, so a Saturday 15:00 result lands by 18:00 at the latest */
